...
// Get quiz details for admin editing (includes correct answers)
router.get('/:quizId/admin', requireAdmin, async (req, res) => {
    try {
        const quiz = await Quiz.findById(req.params.quizId)
            .populate('courseId', 'title');
        ...
        console.error('Error fetching quiz for admin:', error);
        res.status(500).json({ error: 'Failed to fetch quiz' });
    }
});
