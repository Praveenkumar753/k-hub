...
function executeCode(code, language, input, timeLimit) {
    return new Promise((resolve) => {
        ...
        // Map language names to compilex format
        const langMap = {
            'c': 'C',
            'cpp': 'Cpp',
            'java': 'Java',
        ...
        const compilexLang = langMap[language];
        if (!compilexLang) {
        ...
        try {
            if (language === 'javascript') {
                ...
                compilex.compileJavaScript(code, input, (data) => {
                    ...
                    if (data.error) {
                        ...
                        resolve({
                            ...
                            error: '',
                            executionTime,
                            memoryUsed: 0, // Memory usage not easily available in Node.js
                            output: data.output || ''
                        });
                    }
                });
            } else if (language === 'python') {
                // For Python execution - create a wrapper that handles input properly
                const inputStr = String(input || '');
                console.log('🐍 Python execution with input:', JSON.stringify(inputStr));
                
                // Create a Python script that simulates input
                let pythonCodeWithInput = code;
                if (inputStr.trim()) {
                    // Replace input() calls with predefined input
                    const inputLines = inputStr.trim().split('\n');
                    
                    // Simple approach: if code contains input(), replace with hardcoded values
...
${code}`;
                ...
                console.log('🐍 Executing Python code:', pythonCodeWithInput);
                
                // Use the correct compilex method signature for Python
                compilex.compilePython('', pythonCodeWithInput, (data) => {
                    const executionTime = Date.now() - startTime;
                    
                    console.log('🐍 Python execution result:', JSON.stringify(data, null, 2));
                    console.log('🐍 Python output:', JSON.stringify(data.output));
                    console.log('🐍 Python error:', JSON.stringify(data.error));
                ...
                // For other languages (C, C++, Java)
                const compileMethod = `compile${compilexLang}`;
                
                if (typeof compilex[compileMethod] !== 'function') {
                    return resolve({
                        status: 'error',
                        error: 'Compilation method not found',
                        executionTime: 0,
                        memoryUsed: 0,
                        output: ''
                    });
                ...
                compilex[compileMethod](code, input, (data) => {
                    ...
                    if (data.error) {
                        ...
                        resolve({
                            ...
                            error: '',
                            executionTime,
                            memoryUsed: 0, // Memory usage tracking would need additional implementation
                            output: data.output || ''
                        });
                    }
                });
            ...
            }, timeLimit + 1000); // Add 1 second buffer
